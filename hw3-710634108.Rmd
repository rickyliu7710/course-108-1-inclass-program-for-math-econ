...
```{python ans11}
df_jointP = pd.DataFrame(joint_probability, index = hw3data['weather index'],columns = hw3data['mood index'])
df_jointP
...
```{python ans12}
marginP_weather = {
    "sunny" : df_jointP.loc['sunny'].sum(),
'cloudy' : df_jointP.loc['cloudy'].sum(),
'rainy' : df_jointP.loc['rainy'].sum()}

marginP_mood = {
    "happy" : df_jointP.loc[:,'happy'].sum(),
    "fair" : df_jointP.loc[:,'fair'].sum(),
    "sad" : df_jointP.loc[:,'sad'].sum(),
    "depressed" : df_jointP.loc[:,'depressed'].sum()
}
...
```{python ans13}
list_marginP_wearther = [marginP_weather['sunny'],marginP_weather['cloudy'],marginP_weather['rainy']]
P_mood_cond_weather = df_jointP.div(list_marginP_wearther,axis=0)
list_marginP_mood = [marginP_mood["happy"],marginP_mood['fair'],marginP_mood['sad'],marginP_mood['depressed']]
P_weather_cond_mood = df_jointP.div(list_marginP_mood,axis = 1)
...
```{python ans14}
p_ans14 = P_mood_cond_weather.iloc[0,2]
q1ans['ans14']=p_ans14
```
...
```{python ans15}
est_mood = P_mood_cond_weather.loc['cloudy'].max()
...
```{python ans21}
x_partition = np.linspace(2,3,num = 1401)
...
```{python ans22}
def x_star_calculation(list_file, alpha = 0.5):
    x_star = []
    if alpha >1:
        return "error"
    if alpha < 0:
    ...
    for i in range(len(list_file)-1):
        each_x_star = x_partition[i]+alpha*(x_partition[i+1]-x_partition[i])
        x_star.append(each_x_star)
    return x_star


x_star = x_star_calculation(x_partition)
# q2ans['x_star']=x_star
```
...
for i in range(len(x_partition)-1):
    x_value = f(x_star[i])*(x_partition[i+1]-x_partition[i])
    ind_num.append(x_value)
    sum += x_value
    accu_num.append(sum)
rsum = np.array(ind_num).sum()
q2ans['ans23']=rsum
```
