...
```{python ans12}
import itertools
A_big=set([frozenset([]),frozenset(Omega)])

maxN=len(Omega)
for outcomeNumber in range(1,maxN):
    for ix in itertools.combinations(Omega, outcomeNumber): # (1)
...
```{python ans13}
def P_measure(A,Omega={1,2,3,4,5,6}):
    Omega = np.array(list(Omega))
    
    P_sampleSpace = pd.Series(
        np.array([1,3,4,2,6,3])/sum(np.array([1,3,4,2,6,3])),
        index=np.arange(1,7))
    
    P_omega=pd.Series(P_sampleSpace, index=Omega)
