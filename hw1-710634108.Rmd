...
```{python ans12}
x_str = [str(x[i])for i in range(12)]
...
```{python ans14}
x_logi = [x[i] <50 for i in range(12)]
x_logi# x_logi
```
...
```{python ans21}
num_taoyuan = ["桃園市" in dict_business["公司所在地"][i] for i in range(3446)].count(True)
num_taoyuan # num_taoyuan
```

### 2.2
有多少公司的'資本額'大於500000，請將數字存在`num_largeCapital`。
```{python ans22}
num_largeCapital = [dict_business["資本額"][i] > 500000 for i in range(len(dict_business["資本額"]))].count(True)
num_largeCapital# num_largeCapital
```
...
for i in range(len(danceInfo)):
    title_sample = danceInfo[i]['title']
    time_sample = danceInfo[i]['showInfo'][0]['time']
    local_sample = danceInfo[i]['showInfo'][0]['location']
    dic1 = {"title":title_sample,"time":time_sample,"local":local_sample}
    danceInfoList.append(dic1)
danceInfoList# danceInfoList
```
...
```{python ans36}
danceInfoStr= []
for i in range(len(danceInfoList)):
    num = '【{0}】 將於 {1} 在 {2} 演出'.format(danceInfoList[i]['title'],danceInfoList[i]['time'],danceInfoList[i]['local'])
    danceInfoStr.append(num)
danceInfoStr# danceInfoStr
```
